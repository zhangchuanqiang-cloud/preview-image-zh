...
var require_tmp = __commonJS({
  "node_modules/tmp/lib/tmp.js"(exports2, module2) {
    ...
    var _c = { fs: fs8.constants, os: os.constants };
    ...
    function _assertIsRelative(name, option, tmpDir) {
      if (option === "name") {
        ...
        let basename2 = path7.basename(name);
    ...
    function _getTmpDir(options) {
      return path7.resolve(options && options.tmpdir || os.tmpdir());
    }
...
var acceptedExtensions = [".svg", ".png", ".jpeg", ".jpg", ".bmp", ".gif", ".ico", ".webp", ".avif", ".jfif"];
...
function copyFile(source, target, cb) {
  const fileSizeInBytes = fs2.statSync(source).size;
...
// src/util/currentColorHelper.ts
var fs3 = __toESM(require("fs"));
var import_util = require("util");
function replaceCurrentColorInDataURI(uri, currentColor2) {
  ...
  if (uri.startsWith(base64SVGPrelude)) {
    ...
    uri = base64SVGPrelude + encodeURIComponent(btoa(original.replace("<svg", `<svg style="color:${currentColor2}"`)));
    return uri;
  }
  ...
  if (uri.startsWith(defaultEncodingSvgPrelude)) {
    ...
    uri = base64SVGPrelude + encodeURIComponent(btoa(uri.replace("<svg", `<svg style="color:${currentColor2}"`)));
    return uri;
  }
  return uri;
}
function replaceCurrentColorInFileContent(path7, currentColor2) {
  return new Promise((res, rej) => {
    if (path7.endsWith(".svg") && currentColor2 && currentColor2 != "") {
      const read = (0, import_util.promisify)(fs3.readFile);
      const write = (0, import_util.promisify)(fs3.writeFile);
      read(path7).then((data) => {
        const original = data.toString("utf-8");
        return original.replace("<svg", `<svg style="color:${currentColor2}"`);
      }).then((data) => {
        return write(path7, data);
      }).then(() => res(path7)).catch((err) => rej(err));
    } else {
      res(path7);
    }
  });
...
var ImageCache = {
  ...
  delete: (key) => {
    imageCache.get(key).then((tmpFile) => fs4.unlink(tmpFile, () => {
    }));
        ...
        const promise = new Promise((resolve, reject) => {
          if (absoluteImageUrl.scheme && absoluteImageUrl.scheme.startsWith("http")) {
            fetch(new url.URL(absoluteImagePath).toString(), {
              ...
              const dest = fs4.createWriteStream(filePath);
              resp.body.pipe(dest);
            ...
            try {
              const handle = fs4.watch(absoluteImagePath, function fileChangeListener() {
                handle.close();
                fs4.unlink(filePath, () => {
                });
  ...
  cleanup: () => {
    imageCache.forEach((value) => {
      value.then((tmpFile) => fs4.unlink(tmpFile, () => {
      }));
    });
    imageCache.clear();
  }
...
var relativeToOpenFileUrlMapper = {
  map(fileName, imagePath, additionalMetadata) {
    let absoluteImagePath;
    const pathName = path2.normalize(imagePath);
    if (pathName) {
      let testImagePath = path2.join(fileName, "..", additionalMetadata?.relativeImageDir || "", pathName);
...
var RelativeToWorkspaceRootFileUrlMapper = class {
    ...
    if (this.workspaceFolder) {
      ...
      if (pathName) {
        ...
        for (let index = 0; index < pathsToTest.length; index++) {
          ...
          if (ImageCache.has(testImagePath) || fs6.existsSync(testImagePath)) {
            absoluteImagePath = testImagePath;
          } else if (this.additionalSourceFolders.length > 0) {
            for (let i2 = 0; i2 < this.additionalSourceFolders.length; i2++) {
              const additionalSourceFolder = this.additionalSourceFolders[i2];
              let testImagePath2;
              if (path3.isAbsolute(additionalSourceFolder)) {
                testImagePath2 = path3.join(additionalSourceFolder, testPath);
              } else {
                testImagePath2 = path3.join(rootPath, additionalSourceFolder, testPath);
              }
              if (ImageCache.has(testImagePath2) || fs6.existsSync(testImagePath2)) {
  ...
  refreshConfig(workspaceFolder, sourcefolders, paths) {
...
var simpleUrlMapper = {
  map(fileName, imagePath) {
    ...
    if (imagePath.indexOf("http") == 0) {
      ...
      if (ImageCache.has(imagePath) || fs7.existsSync(imagePath)) {
        absoluteImagePath = imagePath;
      }
    }
    return absoluteImagePath;
...
var schemePrefix = "([a-zA-Z]{2,}:)";
var pathPrefix = "(\\.\\.?|\\~)";
var pathSeparatorClause = "\\/";
var excludedPathCharactersClause = "[^\\0\\s!$`&*()\\[\\]+'\":;\\\\]";
var unixLocalLinkClause = "((" + schemePrefix + "|" + pathPrefix + "|(" + excludedPathCharactersClause + ")+)?(" + pathSeparatorClause + "(" + excludedPathCharactersClause + ")+)+)";
...
var winLocalLinkClause = "((" + schemePrefix + "|" + winPathPrefix + "|(" + winExcludedPathCharactersClause + ")+)?(" + winPathSeparatorClause + "(" + winExcludedPathCharactersClause + ")+)+)";
...
// src/recognizers/srcidrecognizer.ts
var path5 = __toESM(require("path"));
var SrcIdRecognizer = class {
  constructor() {
    // 匹配任何包含 src 和 srcid 属性的标签
    this.imageRegex = /<[^>]*src=["']([^"']+)["'][^>]*srcid=["']([^"']+)["'][^>]*>/g;
  }
  recognize(lineIndex, line) {
    const matches = [];
    let match;
    while ((match = this.imageRegex.exec(line)) !== null) {
      const imagePath = match[1];
      const srcId = match[2];
      const baseName = path5.parse(imagePath).name;
      const ext = path5.parse(imagePath).ext;
...
// src/recognizers/varexpsrcrecognizer.ts
var VarExpSrcRecognizer = class {
  constructor() {
    // 存储变量定义
    this.varMap = /* @__PURE__ */ new Map();
    // 匹配变量定义
    this.varRegex = /<Var[^>]*name=["']([^"']+)["'][^>]*expression=["']([^"']+)["'][^>]*>/g;
    // 匹配使用变量的图片标签
    this.imageRegex = /<[^>]*srcExp=["']@([^"']+)["'][^>]*>/g;
  }
  recognize(lineIndex, line) {
    const matches = [];
    let varMatch;
...
var srcIdRecognizer = new SrcIdRecognizer();
var varExpSrcRecognizer = new VarExpSrcRecognizer();
var recognizers = [
  varExpSrcRecognizer,
  // 放在最前面，优先处理变量引用
  srcIdRecognizer,
...
async function collectEntries(document, request, cancellationToken) {
  ...
  absoluteUrlMappers.forEach(
    (absoluteUrlMapper) => absoluteUrlMapper.refreshConfig(request.workspaceFolder, request.additionalSourcefolders, request.paths)
  );
      ...
      urlMatches.forEach((urlMatch) => {
        ...
        items = items.concat(
          Array.from(absoluteUrlsSet.values()).map((absoluteImagePath) => {
            const result = convertToLocalImagePath(
              absoluteImagePath,
              urlMatch,
...
async function convertToLocalImagePath(absoluteImagePath, urlMatch, urlDetectionPatterns = [], currentColor2) {
  if (absoluteImagePath) {
    ...
    if (!isDataUri) {
      ...
      if (absoluteImageUrl && absoluteImageUrl.path) {
        let absolutePath = path6.parse(absoluteImageUrl.path);
    ...
    if (isDataUri || isExtensionSupported || isPatternSupported) {
      if (isDataUri) {
        return Promise.resolve({
          ...
          imagePath: replaceCurrentColorInDataURI(absoluteImagePath, currentColor2),
          range
        });
      } else {
